...
let make_hand str_list = List.map parse_tile_str str_list
...
let test_win_hand _ =
  ...
  in
  assert_equal (-1)
    (Hand.calculate_shanten hand)
    ~msg:"Standard win should correspond to -1 shanten";
...
let test_get_recommendations _ =
  ...
  assert_equal 0 (List.length recs_empty)
    ~msg:"Should return empty list for 13 tiles"

let test_game_create _ =
  let g = Game.create () in
  assert_equal 0 (Game.current_player_id g)
    ~msg:"Game should start with Player 0";
  ...
  List.iter (fun p -> assert_equal 13 (Player.tile_count p)) players
...
let test_draw_and_discard _ =
  ...
  assert_equal 14
    (Player.tile_count p_drawn)
    ~msg:"Hand size should be 14 after draw";
  assert_equal 69
    (Game.remaining_tiles g_drawn)
    ~msg:"Deck count should decrease";
  ...
  assert_equal 1
    (Game.current_player_id g_discarded)
    ~msg:"Turn should rotate to Player 1"
...
let test_bot_step _ =
  ...
  assert_equal 2
    (Game.current_player_id g_next)
    ~msg:"Turn should rotate to Player 2 after bot move";
  assert_equal 69
    (Game.remaining_tiles g_next)
    ~msg:"Bot should have consumed 1 card"
...
let test_last_discard_logic _ =
  ...
  assert_equal None (Game.last_discard g)
    ~msg:"Should have no discards at start";
  ...
  assert_equal 1
    (Game.current_player_id g_after_discard)
    ~msg:"Turn should rotate to Player 1";
  ...
  | Some t ->
      assert_equal
        (Tile.to_string tile_to_discard)
        (Tile.to_string t)
...
let test_game_perform_chi _ =
  ...
  assert_equal 0
    (Game.current_player_id g_new)
    ~msg:"Turn should remain with Player 0 to discard";
...
let test_game_perform_pon _ =
  ...
  assert_equal 0
    (Game.current_player_id g_new)
    ~msg:"Turn should jump back to Player 0";
...
let test_game_perform_kan _ =
  ...
  assert_equal 2
    (List.length (Player.hand p0_new))
    ~msg:"Should draw Rinshan tile (1 remaining + 1 drawn)"
