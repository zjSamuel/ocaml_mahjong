...
let render_html (game: Game.t) : string =
  ...
  (* A. 生成 AI 助手 HTML (Sidebar Content) *)
  (* 注意：这里去掉了 absolute positioning，改为普通的 div *)
  let suggestion_html =
    if can_discard then
        ...
        top3 |> List.map (fun (tile, count) ->
          Printf.sprintf 
            "<div style='display:flex; align-items:center; margin-bottom:8px; background:rgba(0,0,0,0.2); padding:5px; border-radius:4px;'>
               <span style='margin-right:10px; color:#a29bfe; font-weight:bold; font-size:0.9em;'>打:</span>
               <span class='tile static small' style='margin:0;'>%s</span>
               <div style='margin-left:auto; text-align:right;'>
                 <div style='color:#ccc; font-size:0.8em;'>进张</div>
      ...
      if rows = "" then "" 
        ...
        Printf.sprintf 
          "<div class='sidebar-panel'>
             <div class='sidebar-header'>💡 牌效助手</div>
             %s
           </div>"
          rows
    else 
      (* 如果不需要打牌，显示占位或者空 *)
      "<div class='sidebar-panel' style='opacity:0.5'><div class='sidebar-header'>💡 牌效助手</div><div style='padding:10px; font-size:0.9em; color:#888;'>等待摸牌...</div></div>"
  in
  ...
  let action_buttons_html =
    ...
    | Some target ->
        ...
        if (not is_my_turn) && (last_discarder <> 0) then (
          if Player.can_ron human_p target then btns := (!btns) @ [Printf.sprintf "<form action='/win_ron' method='POST' style='display:inline'><input type='hidden' name='target' value='%s'><button class='win-btn'>⚡ 荣和!</button></form>" t_str];
          if Player.can_pon human_p target then btns := (!btns) @ [Printf.sprintf "<form action='/pon' method='POST' style='display:inline; margin-left:10px;'><input type='hidden' name='target' value='%s'><button class='action-btn' style='background:#0984e3'>碰</button></form>" t_str];
          if Player.can_kan human_p target then btns := (!btns) @ [Printf.sprintf "<form action='/kan' method='POST' style='display:inline; margin-left:10px;'><input type='hidden' name='target' value='%s'><button class='action-btn' style='background:#6c5ce7'>杠</button></form>" t_str];
        );
        ...
        if !btns = [] then "" 
          ...
          if not is_my_turn then base_html ^ "<form action='/bot_move' method='POST' style='display:inline; margin-left:20px;'><button class='action-btn' style='background:#b2bec3'>⏭ 跳过 (Pass)</button></form></div>"
          else base_html ^ "</div>"
        )
  in
  ...
  let auto_play_script =
    if should_auto_play then
      "<script>setTimeout(function() { document.getElementById('bot-auto-form').submit(); }, 1000);</script>
       ...
       <div style='text-align:center; padding:10px; background:#ffeaa7; color:#d63031; font-weight:bold; border-radius:5px; margin-bottom:10px;'>⏳ 玩家 " ^ (string_of_int current_idx) ^ " 正在思考...</div>"
    else ""
  in
  ...
  "<html>
    <head>
      ...
      <style>
        ...
        .game-center { flex: 1; min-width: 0; } /* 左侧游戏区 */
        .sidebar { width: 240px; flex-shrink: 0; display: flex; flex-direction: column; gap: 20px; } /* 右侧侧边栏 */
    ...
    <body>
      ...
      <div class='main-layout'>
        ...
        <div class='sidebar'>
           ...
           <div class='sidebar-panel'>
             <div class='sidebar-header'>🔧 调试信息</div>
  ...
  suggestion_html (* AI 助手放到了 Sidebar *)
  debug_html      (* Debug 信息放到了 Sidebar *)
...
let () = 
  ...
  @@ Dream.router [
    ...
    Dream.post "/play" (fun req -> 
      ...
      | `Ok [("discard_index", i)] -> 
          ...
          if idx < 1 || idx > List.length hl then 
            ...
            (* 注意：这里不自动调用机器人，而是等待前端脚本触发 /bot_move *)
            game_state_ref := ng1; 
            Dream.redirect req "/" 
             ...
             (match parse_tile_str ts with
              | Some tile -> 
                  ...
                  Dream.html (Printf.sprintf 
                      "<html><head><meta charset='utf-8'><title>荣和!</title><style>body { background-color: #2d3436; color: white; font-family: 'Segoe UI', sans-serif; text-align: center; padding-top: 50px; } .card { background: #333; display: inline-block; padding: 40px; border-radius: 15px; box-shadow: 0 10px 30px rgba(0,0,0,0.5); } h1 { color: #fdcb6e; font-size: 4em; margin-bottom: 10px; text-shadow: 0 0 20px #e17055; } h2 { color: #00b894; margin-top: 0; } .loser { color: #ff7675; font-weight: bold; font-size: 1.2em; margin: 20px 0; } a { color: #74b9ff; text-decoration: none; font-size: 1.2em; border: 1px solid #74b9ff; padding: 10px 30px; border-radius: 30px; transition: 0.3s; } a:hover { background: #74b9ff; color: #2d3436; } </style></head><body><div class='card'><h1>⚡ 荣和! Ron! ⚡</h1><h2>🎉 获胜者: %s</h2><div class='loser'>💥 点炮者: %s</div><div style='margin: 30px 0; font-size: 1.5em; letter-spacing: 2px;'>%s <span style='border: 2px solid #fdcb6e; padding: 2px 8px; border-radius: 4px; margin-left: 10px;'>%s</span></div><p style='margin-top: 50px;'><a href='/new_game'>再来一局</a></p></div></body></html>" 
    ...
    Dream.post "/win" (fun _ ->
      let game = !game_state_ref in
      (* 注意：这里我们检查 Human Player (0) 是否自摸，而不是 current_player *)
      (* 因为 current_player 只有在轮到我的时候才是我。如果按钮显示了，说明是我。*)
      let all_p = Game.all_players game in
