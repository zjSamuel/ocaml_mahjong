...
let create () =
  ...
  let players =
    Array.init 4 (fun i -> Player.create (Printf.sprintf "Player%d" i))
  in
...
let last_discard g =
  ...
  match Player.discards prev_p with [] -> None | h :: _ -> Some h
      ...
      then (
          ...
          get_visible_counts
            {
              g with
              players =
                (let arr = Array.copy g.players in
                 arr.(g.current_player_idx) <- p_drawn;
                 arr);
            }
        ...
        | Some tile_to_discard -> (
            ...
            | Some p_after_discard ->
                ...
                let next_g =
                  next_turn { g with deck = deck_after_draw; players = nps }
                in
                (next_g, true)))
