...
module type Searchable = sig
  type t
  (** The abstract type representing a state in the search problem (e.g., a
      board configuration). *)
  ...
  (** [compare t1 t2] returns an integer indicating the ordering of two states.
      ...
      - Returns a positive integer if [t1] is greater than [t2]. This is
        required for maintaining the 'closed set' to avoid cycles. *)

  val heuristic : t -> float
  (** [heuristic state] estimates the "cost to go" from [state] to the goal. For
      the A* algorithm to guarantee finding the optimal path (shortest
      distance), this heuristic function must be **admissible**, meaning it
      never overestimates the actual cost to reach the goal.
      - h(n) <= actual_cost(n, goal) *)

  val neighbors : t -> (t * float) list
  (** [neighbors state] returns a list of adjacent states reachable from the
      current [state]. Each element in the list is a tuple: (next_state,
      step_cost). *)
...
(** [Make] Functor Constructs a generic A* solver module for a given
    [Searchable] implementation. *)
module Make (S : Searchable) : sig
  val search : S.t -> (float * S.t) option
  (** [search start_state] Executes the A* search algorithm starting from
      [start_state].
