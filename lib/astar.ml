...
module Make (S : Searchable) = struct
  type node = {
    ...
    f : float;
        (** f(n) = g(n) + h(n): The estimated total cost of the path through
            this node. *)
  }
  (** Internal representation of a search node. *)

  (** [sort_queue q] Sorts the priority queue based on the 'f' score in
      ascending order. Nodes with lower estimated total costs are prioritized.
